...
const ManagerRequestsTable = ({ requests, setRequests, filterStatus }) => {
  const [selectedRequest, setSelectedRequest] = useState(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isRejectDialogOpen, setIsRejectDialogOpen] = useState(false);
  const [rejectionRemarks, setRejectionRemarks] = useState('');
  const [actionRequestId, setActionRequestId] = useState(null); // For approve/reject
  ...
  const handleApprove = async (requestId) => {
    ...
    try {
      await TireRequestService.approveRequest(requestId, "Approved by Manager", 'manager-approve');
      toast.success('Request approved successfully!');
      // Update request status locally or re-fetch
      setRequests(prevRequests =>
        prevRequests.map(req =>
          req.id === requestId ? { ...req, status: 'MANAGER_APPROVED', managerApproved: true } : req
        )
      );
    } catch (error) {
  ...
  const handleRejectSubmit = async () => {
    ...
    try {
      await TireRequestService.rejectRequest(actionRequestId, rejectionRemarks, 'manager-reject');
      toast.success('Request rejected successfully!');
      setRequests(prevRequests =>
        prevRequests.map(req =>
          req.id === actionRequestId ? { ...req, status: 'REJECTED', managerRemarks: rejectionRemarks } : req
        )
      );
      closeRejectDialog();
    } catch (error) {
  ...
  if (!requests || requests.length === 0) {
    return <Typography sx={{mt:2}}>No tire requests found.</Typography>;
  }

  return (
    <Box>
      <TableContainer component={Paper} sx={{mt: 2}}>
        <Table sx={{ minWidth: 650 }} aria-label="tire requests table">
          <TableHead>
              ...
              <TableRow key={request.id} hover>
                ...
                <TableCell align="center">
                  ...
                  {request.status === 'PENDING' && (
                    <>
                      <IconButton onClick={() => handleApprove(request.id)} color="success" size="small" title="Approve">
      ...
      <Dialog open={isRejectDialogOpen} onClose={closeRejectDialog}>
        <DialogTitle>Reject Tire Request</DialogTitle>
        <DialogContent>
          <DialogContentText>
            Please provide remarks for rejecting this request (Vehicle: {requests.find(r=>r.id === actionRequestId)?.vehicleNo}).
          </DialogContentText>
          <TextField
            autoFocus
            margin="dense"
